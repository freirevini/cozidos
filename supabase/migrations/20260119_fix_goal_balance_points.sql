-- ============================================================
-- MIGRATION: Correção do Cálculo de Saldo de Gols (Por Partida)
-- Data: 2026-01-19
-- ============================================================
-- 
-- NOVA REGRA: Saldo de Gols calculado POR PARTIDA
--   - Se o time VENCEU: +N pontos (diferença de gols)
--   - Se o time PERDEU ou EMPATOU: +0 pontos (não subtrai)
--
-- Exemplo:
--   Partida 1: Ganhou 5x0 -> +5 pts
--   Partida 2: Perdeu 0x2 -> +0 pts
--   Partida 3: Ganhou 1x0 -> +1 pt
--   Total: 6 pontos (antes seria 5-2+1 = 4)
--
-- ============================================================
...
DECLARE
  ...
  v_goal_balance_bonus INT;       -- NOVO: Bônus de saldo por vitória
  v_clean_sheets INT;
...
BEGIN
  ...
  LOOP
    ...
    v_goal_difference := 0;
    v_goal_balance_bonus := 0;  -- NOVO: Inicializar bônus
    v_clean_sheets := 0; 
      ...
      LOOP
        ...
        IF v_player_team IS NOT NULL THEN
          ...
          IF v_team_goals > v_opponent_goals THEN 
            ...
            -- NOVA REGRA: Bônus de saldo APENAS em vitórias
            v_goal_balance_bonus := v_goal_balance_bonus + (v_team_goals - v_opponent_goals);
          ELSIF v_team_goals = v_opponent_goals THEN 
            v_draws := v_draws + 1;
            -- Empate: +0 de bônus de saldo
          ELSE 
            v_defeats := v_defeats + 1;
            -- Derrota: +0 de bônus de saldo (NÃO subtrai)
          END IF;
          
          -- Clean Sheet: apenas se NÃO tomou gol E marcou pelo menos 1 (0x0 não conta)
    ...
    -- ===== CALCULAR PONTOS (REGRAS 2026 ATUALIZADAS) =====
    
    IF v_is_absent THEN
      -- FALTA: -20 pontos, 0 ranking
      v_presence_points := 0;
      v_victory_points := 0;
      v_draw_points := 0;
      v_goal_points := 0;
    ...
    ELSIF v_is_substitute THEN
      -- Substituto: 0 pontos de ranking
      v_presence_points := 0;
      v_victory_points := 0;
      v_draw_points := 0;
      v_goal_points := 0;
    ...
    ELSE
      ...
      -- NOVA LÓGICA: Bônus de saldo (apenas vitórias) + Clean Sheets
  ...
  RETURN json_build_object(
    'success', true, 
    'message', format('Recalculados %s jogadores com nova regra de saldo', v_player_count),
...
'Regras 2026 v2: Presença +3, Vitória +3, Empate +1, Saldo APENAS em vitórias (+N), CS +2 (não 0x0), Atraso -10, FALTA -20, Amarelo -1, Azul -2';
