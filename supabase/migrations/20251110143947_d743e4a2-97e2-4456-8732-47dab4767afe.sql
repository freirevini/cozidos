-- Fix recalc_round_aggregates to use correct enum value 'falta' instead of 'ausente'
...
BEGIN
  ...
  LOOP
    ...
    -- Count cards (using correct Portuguese enum values)
    SELECT 
    ...
    -- Count attendance issues (using correct enum values: 'atrasado' and 'falta')
    SELECT 
      COALESCE(COUNT(*) FILTER (WHERE status = 'atrasado'), 0),
      COALESCE(COUNT(*) FILTER (WHERE status = 'falta'), 0)
    INTO lates, absences
