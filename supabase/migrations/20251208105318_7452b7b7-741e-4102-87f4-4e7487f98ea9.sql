-- Garantir que os campos necessários existam em profiles
-- (os campos claim_token, claim_token_used_at, created_by_admin_simple já foram criados na migration anterior)

-- Atualizar a função generate_claim_token com tratamento de unique_violation (loop)
...
-- Atualizar claim_profile_with_token para garantir is_player = true
CREATE OR REPLACE FUNCTION public.claim_profile_with_token(p_token text, p_user_id uuid)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
-- Atualizar admin_link_pending_to_profile para garantir is_player = true
