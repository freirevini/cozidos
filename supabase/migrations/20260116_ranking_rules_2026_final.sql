-- ============================================================
-- REGRAS DE CLASSIFICAÇÃO 2026 - VERSÃO FINAL
-- Data: 2026-01-16
-- ============================================================
-- 
-- REGRAS:
...
--   Vitória: +3 cada
--   Empate: +1 cada
--   Saldo de Gols: +N (total da rodada, apenas se positivo)
--   Clean Sheet: +2 cada (apenas se venceu sem tomar gol, 0x0 não conta)
--   Atraso: -10 cada
--   FALTA: -20 cada (+ 0 pontos de ranking)
...
BEGIN
  ...
  LOOP
    ...
    v_goal_difference := 0; v_clean_sheets := 0; 
    ...
    IF NOT v_is_absent THEN
      ...
      LOOP
        ...
        IF v_player_team IS NOT NULL THEN
          ...
          -- Saldo de gols acumulado da rodada
    ...
    ELSE
      ...
      v_goal_points := GREATEST(v_goal_difference, 0) + (v_clean_sheets * 2);
      v_absence_points := 0;
    END IF;
  ...
  RETURN json_build_object(
    'success', true, 
    'message', format('Recalculados %s jogadores', v_player_count),
...
'Regras 2026: Presença +3, Vitória +3, Empate +1, SG+ = +N, CS +2 (não 0x0), Atraso -10, FALTA -20, Amarelo -1, Azul -2';
