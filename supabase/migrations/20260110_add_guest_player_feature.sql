-- Migration: Adiciona suporte a Jogador Avulso (Guest Player)
-- Jogadores avulsos podem registrar eventos (gols, cartões) mas NÃO aparecem na classificação

-- 1. Adicionar coluna is_guest na tabela profiles
ALTER TABLE public.profiles 
ADD COLUMN IF NOT EXISTS is_guest BOOLEAN DEFAULT false;

-- 2. Criar índice para melhorar performance das queries que filtram por is_guest
CREATE INDEX IF NOT EXISTS idx_profiles_is_guest ON public.profiles(is_guest);

-- 3. Comentário explicativo
COMMENT ON COLUMN public.profiles.is_guest IS 'Indica se o jogador é um convidado temporário (avulso). Jogadores avulsos podem participar de partidas mas não aparecem na classificação geral.';

-- 4. Atualizar função recalc_all_player_rankings para excluir jogadores avulsos
...
BEGIN
  ...
  -- Processar cada jogador aprovado (EXCLUINDO jogadores avulsos)
  FOR profile_record IN
    ...
    WHERE pr.is_player = true 
      ...
      AND (pr.is_guest = false OR pr.is_guest IS NULL) -- NOVA CONDIÇÃO: Excluir avulsos
  LOOP
    ...
    v_final_pontos := (v_final_presencas * 10) +
                      (v_final_vitorias * 3) +
                      (v_final_empates * 1) +
                      (v_final_gols * 1) +
                      (v_final_assistencias * 2) +
                      (v_final_atrasos * -5) +
                      (v_final_faltas * -10) +
                      (v_final_amarelos * -1) +
                      (v_final_azuis * -2) +
...
-- 5. Criar função para adicionar jogador avulso
...
BEGIN
  ...
  RETURN json_build_object(
    ...
    'message', 'Jogador avulso criado com sucesso'
  );
END;
$function$;
