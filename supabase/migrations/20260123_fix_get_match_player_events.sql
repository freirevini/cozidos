-- Migration: Update get_match_player_events to include absence substitutes
-- Purpose: Fix consistency between timeline and badges - both should include players
--          who entered via match_absence_substitutes table
-- Related: get_match_timeline_events already has this fallback
...
BEGIN
  ...
  WITH match_players AS (
    -- 1. Players from original roster (round_team_players)
    ...
    -- 2. Players who entered via substitution during match
    SELECT DISTINCT s.player_in_id as player_id, s.team_color
    FROM substitutions s
    WHERE s.match_id = p_match_id
    
    UNION
    
    -- 3. Players who entered as absence substitutes (before match)
    SELECT DISTINCT mas.substitute_player_id as player_id, mas.team_color
    FROM match_absence_substitutes mas
    WHERE mas.match_id = p_match_id
  ),
  player_subs AS (
    -- Get substitution info for each player (entered during match)
    SELECT 
    ...
    WHERE s.match_id = p_match_id
    UNION ALL
    -- Players not substituted in are starters (includes absence substitutes who start)
    SELECT 
      mp.player_id,
...
COMMENT ON FUNCTION get_match_player_events IS 
'Returns event counts (goals, cards, substitution info) per player for a match. 
Includes players from: 1) original roster, 2) in-match substitutions, 3) absence substitutes.
Used by MatchLineups component for player badges.';
