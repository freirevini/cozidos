...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE id = p_round_id AND status = 'finalizada'
  ) THEN
    RETURN json_build_object('success', false, 'error', 'Rodada não está finalizada. Finalize a rodada antes de recalcular.');
  END IF;
