-- Create trigger to recalculate player_rankings when a round is deleted
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(SUM(prs.red_cards), 0) as cartoes_vermelhos,
...
-- Create trigger that executes after deleting a round
CREATE TRIGGER recalc_rankings_after_round_delete
AFTER DELETE ON public.rounds
FOR EACH ROW
EXECUTE FUNCTION public.recalc_rankings_on_round_delete();

-- Create optional helper function for manual recalculation
